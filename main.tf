...
resource "aws_cloudwatch_metric_alarm" "elasticache_high_cpu" {
  alarm_name          = "${var.prefix}elasticache-high-cpu-${var.cache_cluster_id}"
  alarm_description   = "CPU utilization on ${var.cache_cluster_id} reached > ${var.cpu_percent_threshold}%"
...
resource "aws_cloudwatch_metric_alarm" "elasticache_swap_usage" {
  alarm_name          = "${var.prefix}elasticache-swap-usage-${var.cache_cluster_id}"
  alarm_description   = "Swap usage for ${var.cache_cluster_id} have been greater than ${var.swap_threshold / 1024 / 1024}MB for at least 10 minutes"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
...
resource "aws_cloudwatch_metric_alarm" "elasticache_evictions" {
  alarm_name          = "${var.prefix}elasticache-evictions-${var.cache_cluster_id}"
  alarm_description   = "Evictions for ${var.cache_cluster_id} have been greater than ${var.evictions_threshold} for at least 10 minutes"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
...
resource "aws_cloudwatch_metric_alarm" "elasticache_freeable_memory_low" {
  ...
  alarm_name          = "${var.prefix}elasticache-freeable-memory-low-${var.cache_cluster_id}"
  alarm_description   = "FreeableMemory for ${var.cache_cluster_id} has been lower than ${var.freeable_memory_minimum} for at least 10 minutes"
...
resource "aws_cloudwatch_metric_alarm" "elasticache_connection_count_anomalous" {
  ...
  alarm_name          = "${var.prefix}elasticache-connectioncount-anomaly-${var.cache_cluster_id}"
  comparison_operator = "GreaterThanUpperThreshold"
  evaluation_periods  = var.anomaly_evaluation_periods
  threshold_metric_id = "e1"
  alarm_description   = "Anomalous database connection count detected. Something unusual might be happening."
  alarm_actions       = var.sns_topic_alarm_arns
  ok_actions          = var.sns_topic_ok_arns
...
resource "aws_cloudwatch_metric_alarm" "elasticache_cloudwatch_alarm_currconnections" {
  ...
  alarm_name          = "${var.prefix}elasticache-connectioncount-max-${var.cache_cluster_id}"
